...
export default function MapaPublicoView({ municipalitySlug }: MapaPublicoViewProps) {
  ...
  useEffect(() => {
    const url = municipalitySlug
      ? `/api/mapa-publico?slug=${encodeURIComponent(municipalitySlug)}`
      : '/api/mapa-publico'
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md text-center">
          <p className="text-lg font-semibold text-gray-800">Municipalidad no encontrada</p>
          <p className="text-gray-500 text-sm mt-1">Verifica el enlace que estás usando.</p>
        </div>
      </div>
    )
  }
      ...
      <header className="bg-slate-900 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-xs">
            <a href={reportarHref} className="text-slate-300 hover:text-white underline">
              Reportar emergencia
            </a>
